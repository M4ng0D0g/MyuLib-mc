...
public final class CubeGenerator implements IShape {
    ...
    public List<Vec3d> getOutlinePoints(Vec3d size, double density) {
        ...
        Vec3d half = size.multiply(0.5);
        double hx = half.x;
        double hy = half.y;
        double hz = half.z;
        Vec3d[] corners = new Vec3d[]{
                new Vec3d(-hx, -hy, -hz), new Vec3d(hx, -hy, -hz), new Vec3d(hx, -hy, hz), new Vec3d(-hx, -hy, hz),
                new Vec3d(-hx, hy, -hz), new Vec3d(hx, hy, -hz), new Vec3d(hx, hy, hz), new Vec3d(-hx, hy, hz)
        };
        for (int i = 0; i < 4; i++) {
    ...
    private static void drawLine(List<Vec3d> points, Vec3d start, Vec3d end, double density) {
